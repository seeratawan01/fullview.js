  ...
  $.extend(FullView.prototype, {
    ...
    utilites: function () {
      this.createDots = function createDots () {
        ...
        const div = $('<div>').attr('id', 'fv-dots').append('<ul>')

        if (this.options.dotsTooltips === true) {
          const plugin = this
          this.$views.each(function (i) {
            const tooltipTitle = plugin.$views.eq(i).attr('data-tooltip') ? plugin.$views.eq(i).attr('data-tooltip') : null
            if (tooltipTitle) {
    ...
    settingUp: function () {
      ...
      this.$views.each(function (i) {
        const anchor = this.$views.eq(i).attr('id')
        const viewOffset = this.$views.eq(i).offset().top
      ...
      // Setting Menu
      if (this.$navbar !== null) {
        const $menu = this.$navbar
        const seletedAnchor = this.offsets.filter(function (obj) {
          if (obj.anchor !== undefined) {
            const $menuitem = $menu.find('a[href="#' + obj.anchor + '"]')
            if ($menuitem.length) {
        ...
        const queryString = "a[href='" + seletedAnchor.join("'], a[href='") + "']"
      ...
      const $actview = this.$views.filter('[class="active"]')

      // this.$views.removeClass('active');
      if ($actview.length) {
        const offset = $actview.eq(0).offset().top

        const activeData = this.offsets.filter(function (obj) {
    ...
    callback: function (eventName, addtional) {
      ...
      const onScrollStart = this.options.onScrollStart
      const onScrollEnd = this.options.onScrollEnd
